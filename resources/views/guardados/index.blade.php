...
@section('title', 'Elementos Guardados')

@section('content')
<div class="feed-header">
    <p class="feed-header__eyebrow">Tu colección</p>
    <h1 class="feed-header__title">Elementos Guardados</h1>
    <p class="feed-header__sub">{{ $publicaciones->count() }} zona{{ $publicaciones->count() !== 1 ? 's' : '' }} guardada{{ $publicaciones->count() !== 1 ? 's' : '' }}</p>
</div>

@if($publicaciones->isEmpty())
    <div class="empty-state">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="40" height="40" style="color:var(--c-text-muted);margin-bottom:.75rem">
            <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/>
        </svg>
        <p>No tienes ninguna zona guardada todavía.</p>
        <a href="{{ route('publicaciones.index') }}" class="btn btn--primary">Explorar zonas</a>
    </div>
@else
